...
public static class PassFileManager
{
    ...
    private static IPassFileCryptoService PassFileCryptoService => EnvironmentContainer.Resolve<IPassFileCryptoService>();
    ...
    public static async Task<IDetailedResult<byte[]>> GetEncryptedDataAsync(PassFileType passFileType, int passFileId, bool oldVersion = false)
    {
        ...
        if (!oldVersion && actual is not null)
        {
            ...
            {
                var res = PassFileCryptoService.Encrypt(actual);
                Debug.Assert(res.Ok);
                return res.Ok 
    ...
    public static PassFile CreateNew(PassFileType ofType, string passPhrase)
    {
        ...
        var invertedPassFileId = (int) AppContext.Current.PassFilesCounter + 1;

        while (ids.Contains(-invertedPassFileId))
        {
            ++invertedPassFileId;
        }
    ...
    private static async Task<IDetailedResult> _SaveListAsync(List<PassFile> list)
    {
        ...
        var minId = Math.Min(list.Min(x => x.Id), -(int) AppContext.Current.PassFilesCounter);

        await AppContext.ApplyAsync(appContext => appContext.PassFilesCounter = (uint)minId);
