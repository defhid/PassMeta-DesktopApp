...
public sealed class GifDecoder : IDisposable
{
    ...
    private int _gctSize, _prevFrame;
    ...
    private void ClearImage() => ClearArea(_gifDimensions);
    ...
    {
        if (!_hasNewFrame || _bitmapBackBuffer is null)
        {
            return;
        }
    ...
    private void ProcessExtensions(ref int curFrame, byte[] tempBuf)
    {
        ...
        switch (extType)
        {
            ...
            case ExtensionType.Application:
                ...
                {
                    while (_fileStream.ReadBlock(tempBuf) > 0)
                    {
                    }

                    SpanToShort(tempBuf.AsSpan(1));
                }
                else
                    _fileStream.SkipBlocks(tempBuf);
